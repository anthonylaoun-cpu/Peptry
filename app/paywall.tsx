...
import { activatePremium } from '../utils/userData';
...
export default function PaywallScreen() {
  ...
  const handleSubscribe = async () => {
    // Activate premium status (preserves all existing user data)
    await activatePremium('weekly');
    
    // Go to premium success celebration screen
    ...
    <LinearGradient colors={['#071018', '#0c1929', '#071018']} style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <Animated.View entering={FadeInDown.delay(100).springify()} style={styles.iconContainer}>
          <LinearGradient colors={['#38bdf8', '#0ea5e9']} style={styles.icon}>
            <Ionicons name="diamond-outline" size={48} color="white" />
          </LinearGradient>
          <View style={styles.sparkle}><Ionicons name="star" size={16} color="white" /></View>
        </Animated.View>
        ...
        <Animated.View entering={FadeInUp.delay(400).springify()} style={styles.features}>
          {features.map((feature, index) => (
            <Animated.View key={feature} entering={FadeInUp.delay(400 + index * 50).springify()} style={styles.featureRow}>
              <View style={styles.checkIcon}><Ionicons name="checkmark-outline" size={14} color="#38bdf8" /></View>
...
const styles = StyleSheet.create({
  ...
  icon: { width: 96, height: 96, borderRadius: 48, alignItems: 'center', justifyContent: 'center', shadowColor: '#38bdf8', shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.6, shadowRadius: 30 },
  ...
  checkIcon: { width: 24, height: 24, borderRadius: 12, backgroundColor: 'rgba(56,189,248,0.2)', alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  ...
  ctaButton: { borderRadius: 16, paddingVertical: 20, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', shadowColor: '#38bdf8', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.5, shadowRadius: 16 },
